public class Array1 {
	 ...
	 * 
	 * firstLast6([1, 2, 6]) → true firstLast6([6, 1, 2, 3]) → true
	 ...
	 * least 1.
	 * 
	 * maxTriple([1, 2, 3]) → 3 maxTriple([1, 5, 3]) → 5 maxTriple([5, 2, 3]) →
	 * 5
	 */
	public static int maxTriple(int[] nums) {
		int first, middle, last, largest;
		first = nums[0];
		middle = nums[nums.length / 2];
		last = nums[nums.length - 1];
		largest = Math.max(first, middle);
	 ...
	 * 
	 * unlucky1([1, 3, 4, 5]) → true unlucky1([2, 1, 3, 4, 5]) → true
	 ...
	 * 
	 * make2([4, 5], [1, 2, 3]) → [4, 5] make2([4], [1, 2, 3]) → [4, 1]
	 * make2([], [1, 2]) → [1, 2]
	 */
	public int[] make2(int[] a, int[] b) {
		final int MAXIMUM_LENGTH = 2;
		int[] newArray = new int[MAXIMUM_LENGTH];
		int lastItem = 0;
		...
		for (int i = 0; i < b.length; i++) {
			if (lastItem < MAXIMUM_LENGTH) {
				newArray[lastItem] = b[i];
			} else {
				break;
			}
			lastItem++;
	 ...
	 * all their elements.
	 * 
	 * plusTwo([1, 2], [3, 4]) → [1, 2, 3, 4] plusTwo([4, 4], [2, 2]) → [4, 4,
	 * 2, 2] plusTwo([9, 2], [3, 4]) → [9, 2, 3, 4]
	 */
	...
	public static int[] makeLast(int[] nums) {
		...
		modifiedNums[modifiedNums.length - 1] = nums[nums.length - 1];
		return modifiedNums;
	}
	 ...
	 * array.
	 * 
	 * front11([1, 2, 3], [7, 9, 8]) → [1, 7] front11([1], [2]) → [1, 2]
	 * front11([1, 7], []) → [1]
	 */
	 ...
	 * are present.
	 * 
	 * frontPiece([1, 2, 3]) → [1, 2] frontPiece([1, 2]) → [1, 2]
	 * frontPiece([1]) → [1]
	 */
	 ...
	 * will be at least 3.
	 * 
	 * midThree([1, 2, 3, 4, 5]) → [2, 3, 4] midThree([8, 6, 7, 5, 3, 0, 9]) →
	 * [7, 5, 3] midThree([1, 2, 3]) → [1, 2, 3]
	 */
	public int[] midThree(int[] nums) {
		int middle = nums.length / 2; // find the middle. ints round up
	...
	public int[] swapEnds(int[] nums) {
		int tempFirst = nums[0], tempLast = nums[nums.length - 1];
		nums[0] = tempLast;
	 ...
	 * 
	 * fix23([1, 2, 3]) → [1, 2, 0] fix23([2, 3, 5]) → [2, 0, 5] fix23([1, 2,
	 * 1]) → [1, 2, 1]
	 */
	 ...
	 * 
	 * maxEnd3([1, 2, 3]) → [3, 3, 3] maxEnd3([11, 5, 9]) → [11, 11, 11]
	 * maxEnd3([2, 11, 3]) → [3, 3, 3]
	 */
	public int[] maxEnd3(int[] nums) {
		int newMax = Math.max(nums[0], nums[nums.length - 1]);
		for (int i = 0; i < nums.length; i++)
			nums[i] = newMax;
		return nums;
	}
	 ...
	 * length 1 or more.
	 * 
	 * makeEnds([1, 2, 3]) → [1, 3] makeEnds([1, 2, 3, 4]) → [1, 4] makeEnds([7,
	 * 4, 6, 2]) → [7, 2]
	 */
